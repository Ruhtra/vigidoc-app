...
export const API_BASE_URL = "http://10.61.213.33:3000";
    ...
    try {
      ...
      if (sessionData) {
        ...
        if (token && parsed.user) {
          console.log(
            `[API Client] Sending headers: x-user-id=${parsed.user.id}, token=${token.substring(0, 10)}...`,
          );
      ...
      console.error(
        "[API Client] Erro ao recuperar token do SecureStore:",
        error,
      );
    }
    return config;
